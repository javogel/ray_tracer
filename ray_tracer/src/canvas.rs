...
impl Canvas {
    ...
    pub fn pixel_at(&self, x: usize, y: usize) -> Color {
        ...
        match self.within_bounds(x, y)  {
           true => {
            let i = (width * y + x) * 3;
            return Color {r: pixels[i] as f32 /255. , g: pixels[i + 1] as f32 /255., b: pixels[i + 2]  as f32 /255. };
    ...
    pub fn save(&self, image_type: ImageType){
        match image_type {
            ImageType::PPM => render::save_as_ppm(&self),
...
pub mod render {
    ...
    pub fn save_as(filepath: &str, data: String){
    ...
    pub fn save_as_ppm(canvas: &Canvas) {
        let data = canvas_to_ppm(canvas);
        save_as("./images/image.ppm", data);
    }
    ...
    fn write_file(name: &str, data: String) -> io::Result<()> {
